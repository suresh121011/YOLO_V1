...
| **WP3.0 — Platform Remediation** | **PASS** | CI matrix (ubuntu+windows × py3.10/3.12) in `ci.yml`; downloader tests (`test_downloaders*.py`); S3 remote configured (`.dvc/config` → `s3://elderly-assistant-mlops/datasets/yolo_v1` — *placeholder at audit time; repointed 2026-07-27 to the provisioned bucket `s3://elderly-assistant-mlops-329117470647-ap-south-1-an/datasets/yolo_v1`, PR #14*); clean-machine reproduction log PASS-partial (2026-07-14, network-restricted) in `docs/04_dataset_engineering/reproduction_log.md`. |
...
| ID | Finding | Detail |
|---|---|---|
| **C-1** ✅ **RESOLVED 2026-07-27** | **Smoke dataset is a single copy** | *Finding as recorded at audit time:* `dvc status` reports *not in cache* for every Phase-2 stage output; `.dvc/cache` is essentially empty (repo was moved between user profiles — `tree.txt` records the previous `C:\Users\sm646\` location); `dvc push` has never run (no AWS credentials). The 188-image smoke dataset + labels exist **only as workspace files on this machine** (plus OneDrive sync). Workspace files still match `dvc.lock` hashes (status shows no "modified" on data dirs), so recovery is `dvc commit` + first `dvc push`. **The push requires AWS credentials — human task, and the hard gate before any real capture collection begins (runbook step 0).**<br><br>**Resolution:** the dataset now exists in three places — workspace cache (`C:\dvc_cache`), a local mirror (`C:\dvc_remote`, 20,487 files pushed), and **S3** (`s3://elderly-assistant-mlops-329117470647-ap-south-1-an/datasets/yolo_v1`, **46,337 objects / 6.21 GB**, `ap-south-1`, SSE-AES256, versioning enabled). `dvc status -c -r storage` reports *"Cache and remote are in sync"*. Config landed in PR #14 (`8dfecea`). Note the dataset is no longer the 188-image smoke build this finding described — it is the 17,888-image build.<br><br>**Re-verified 2026-07-29 (Phase F4/F5), against the full-mode 24,352-image build:** S3 holds **68,301 objects / 7.82 GB**; `dvc status -c` exits **0** for both `localstore` and `storage`; `verify_lock_objects.py --deep` resolves all **67,734** `dvc.lock` objects in cache *and* local mirror. Two caveats on the original wording: the "in sync" *message* was never evidence — RG6 read that stdout and passed over 21,964 un-pushed objects until it was fixed to judge the exit code; and the three copies share one physical drive except for S3, so this closes single-*copy*, not single-*disk*. |
...
| 5 | **First `dvc push`** (configure AWS credentials, then `dvc push`; verify `dvc pull` on scratch clone) — closes C-1 | **Critical (operational)** | S | AWS creds | None | ✅ **Done 2026-07-27** — pushed to `localstore` (20,487 files) and `storage` (46,337 objects / 6.21 GB); `dvc status -c -r storage` in sync. `dvc pull` on a scratch clone **verified 2026-07-27** — fresh `git clone` with no cache fetched `data/processed/split_report` from S3 and the content matched (17,888 images, leakage 0, 0 CRLF). Scope limits — same machine/OS/credentials, one small output, `dvc repro qa_check` not re-run — are recorded in `reproduction_log.md` §"Pull-variant fetch check"; the full clean-machine gate remains open. |
